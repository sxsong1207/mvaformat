...
namespace ARCH {
    bool LoadInterface(MVS::Scene &scene, const std::string fileName) {
        MVSA::Scene obj;
        // serialize in
        if (!MVSA::MVAIO::SerializeLoad(obj, fileName)) return false;
        ...
        FOREACH(i, scene.platforms) {
            const MVSA::Scene::Platform plat = obj.platforms[i];
            ...
            FOREACH(j, platform.cameras) {
                const MVSA::Scene::Platform::Camera &cam = plat.cameras[j];
            ...
            FOREACH(j, platform.poses) {
                const MVSA::Scene::Platform::Pose p = plat.poses[j];
        ...
        FOREACH(i, scene.images) {
            const MVSA::Scene::Image &image = obj.images[i];
        ...
        FOREACH(i, scene.pointcloud.points) {
            const MVSA::Scene::Vertex &vertex = obj.vertices[i];
        ...
        FOREACH(i, scene.pointcloud.normals) {
            const MVSA::Scene::Normal &vertexNormal = obj.verticesNormal[i];
        ...
        FOREACH(i, scene.pointcloud.colors) {
            const MVSA::Scene::Color &vertexColor = obj.verticesColor[i];
        ...
        for (int i = 0; i < nMeshVertices; ++i) {
            const MVSA::Scene::Mesh::Vertex &vertex = obj.mesh.vertices[i];
            MVS::Mesh::Vertex &point = scene.mesh.vertices[i];
            point = vertex.X;
        }

        for (int i = 0; i < nMeshFaces; ++i) {
            const MVSA::Scene::Mesh::Face &face = obj.mesh.faces[i];
        ...
        if (!obj.mesh.vertexNormals.empty()) {
            ...
            for (int i = 0; i < nMeshVertices; ++i) {
                const MVSA::Scene::Mesh::Normal &vertexNormal =
        ...
        if (!obj.mesh.vertexVertices.empty()) {
            ...
            for (int i = 0; i < nMeshVertices; ++i) {
                const MVSA::Scene::Mesh::VertexIdxArr &vertexVertex =
        ...
        if (!obj.mesh.vertexFaces.empty()) {
            ...
            for (int i = 0; i < nMeshVertices; ++i) {
                const MVSA::Scene::Mesh::FaceIdxArr &vertexFaces =
        ...
        if (!obj.mesh.faceNormals.empty()) {
            ...
            for (int i = 0; i < nMeshFaces; ++i) {
                const MVSA::Scene::Mesh::Normal &fNormal = obj.mesh.faceNormals[i];
        ...
        if (!obj.mesh.faceTexcoords.empty()) {
            ...
            for (int i = 0; i < nMeshTexCoords; ++i) {
                const MVSA::Scene::Mesh::TexCoord &texCoord =
        ...
        if(!obj.mesh.textureDiffuses.empty()) {
            const MVSA::Scene::Mesh::Texture &texture =
